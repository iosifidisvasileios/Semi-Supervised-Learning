...
//    val confidenceLevel = args(2).toDouble

//    data = data.union(evaluationData).cache()
//    evaluationData.unpersist()
    //    train till november
    for (counter <- 0 to 10) {
...
//      var unlabeledF1 = unlabeled.filter { line =>
//        val parts = line.split(',')
//        var temp = format.parse(parts(1))
//        temp.getMonth == counter
//      }.map{
...
//      }.cache
      ...
      for(step <- 1 to 2) {
        ...
        //        println("size of training Unigrams: " + trainingSetF0.count())
        //        println("size of training Bigrams : " + trainingSetF1.count())
        //        println("size of unlabeled Unigrams: " + unlabeledF0.count())
        //        println("size of unlabeled Bigrams : " + unlabeledF1.count())

        val resultsHigh_L0 = unlabeledF0.map{ item =>
          (model_F0.predict(item._2), item._3)
        }.cache()

        //         val resultsHigh_L0 = unlabeledF0.filter { item =>
        ...
        //        unlabeledF0 = unlabeledF0.filter { item =>
        //          val output = model_F0.predictProbabilities(item._2)
        //          output(0) < confidenceLevel && output(1) < confidenceLevel
        //        }.cache()
...
//        val resultsHigh_L1 = unlabeledF1.map{ item =>
//          (model_F1.predict(item._3), item._2)
//        }.cache()

        //
        //        val resultsHigh_L1 = unlabeledF1.filter { item =>
        ...
        //        }.cache()
        //
        //        unlabeledF1 = unlabeledF1.filter { item =>
        //          val output = model_F1.predictProbabilities(item._3)
        //          output(0) < confidenceLevel && output(1) < confidenceLevel
        //        }.cache()
        if(step == 1) {
